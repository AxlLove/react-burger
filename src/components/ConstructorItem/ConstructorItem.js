...
const ConstructorItem = ({dragId, index, item, moveCard}) => {
    ...
    const deleteIngredient = (id) => {
        dispatch(ingredientSlice.actions.deleteIngredient(id))
    }
    ...
    return (
<li ref={ref} onDrop={preventDefault} style={{ opacity }} data-handler-id={handlerId} className={`${styles.constructorItem} ${isDragging? styles.dragging : ''}`}>
                        <DragIcon type="primary"/>
                        <ConstructorElement
                            text={name}
                            price={price}
                            thumbnail={image_mobile}
                            handleClose={()=>{deleteIngredient(dragId)}}
                            />
